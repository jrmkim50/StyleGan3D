...
def test_discriminator( G, D, Gs, training_set, resume_snapshot=None, lod=0):
    ...
    lod_in = tf.placeholder(tf.float32, shape=[])
    ...
    for i in range(100):
        ...
        # d_pred, fake_image_1 = tflib.run([d_pred_op, fake_image_1_op], feed_dict={latents_1: latents_1_val, lod_in: 0})
        d_pred_real_, d_pred_fake_, real_image = tflib.run([d_pred_real, d_pred_fake, reals], feed_dict={latents_1: latents_1_val, lod_in: lod})
    ...
    print('D Accuracy : ', tp/100, tn/100, (tp+tn)/200)
...
def training_schedule(
    ...
    lod_training_kimg       = 300,      # Thousands of real images to show before doubling the resolution.
    lod_transition_kimg     = 300,      # Thousands of real images to show when fading in new layers.
    minibatch_base          = 16,       # Maximum minibatch size, divided evenly among GPUs.
    minibatch_dict          = {},       # Resolution-specific overrides.
    max_minibatch_per_gpu   = {},       # Resolution-specific maximum minibatch size per GPU.
    G_lrate_base            = 0.005,    # Learning rate for the generator.
    G_lrate_dict            = {},       # Resolution-specific overrides.
    D_lrate_base            = 0.001,    # Learning rate for the discriminator.
    D_lrate_dict            = {},       # Resolution-specific overrides.
    lrate_rampup_kimg       = 0,        # Duration of learning rate ramp-up.
    tick_kimg_base          = 1,      # Default interval of progress snapshots.
    tick_kimg_dict          = {4: 160, 8:140, 16:120, 32:100, 64:60, 128:40, 256:20, 512:30, 1024:20}): # Resolution-specific overrides
...
def training_loop(
    ...
    grid_size, grid_reals, grid_labels_1, grid_latents_1 = misc.setup_snapshot_image_grid(G, training_set, **grid_args)

    if test:
        grid_size, grid_reals, grid_labels, grid_latents = misc.setup_test_snapshot_image_grid(G, training_set, **grid_args)
    ...
    grid_fakes_1 = Gs.run(grid_latents_1, grid_labels_1, is_validation=True, minibatch_size=sched.minibatch//submit_config.num_gpus)
    ...
    misc.save_mri_image(grid_reals, os.path.join(submit_config.run_dir, 'reals.nii.gz'), drange=training_set.dynamic_range)
    misc.save_mri_image(grid_fakes, os.path.join(submit_config.run_dir, 'fakes%06d.nii.gz'%0), drange=drange_net)
    misc.save_mri_image(grid_fakes_1, os.path.join(submit_config.run_dir, 'fakes%06d_1.nii.gz'%0), drange=drange_net)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        for _mb_repeat in range(minibatch_repeats):
            ...
            for g_repeat in range(1):
                tflib.run([G_train_op], {lod_in: sched.lod, lrate_in: sched.G_lrate, minibatch_in: sched.minibatch})
                cur_nimg += sched.minibatch

        # Perform maintenance tasks once per tick.
        d_loss,g_loss = tflib.run([D_loss, G_loss], {lod_in: sched.lod, lrate_in: sched.D_lrate, minibatch_in: sched.minibatch})
        d_loss_total+=np.array(d_loss).mean()
        g_loss_total+=np.array(g_loss).mean()
        count+=1
        print(count)
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            g_loss_total = g_loss_total/count
            d_loss_total = d_loss_total/count

            # Report progress.
            print('tick %-5d kimg %-8.1f lod %-5.2f  g loss %-2.5f d loss %-2.5f minibatch %-4d time %-12s sec/tick %-7.1f sec/kimg %-7.2f maintenance %-6.1f gpumem %-4.1f' % (
            ...
            # d_acc = test_discriminator(G, D, Gs, training_set, sched.lod)
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                ...
                misc.save_mri_image(grid_fakes, os.path.join(submit_config.run_dir, 'fakes%06d.nii.gz'%(cur_nimg // 1000)), drange=drange_net)
                misc.save_mri_image(grid_fakes_1, os.path.join(submit_config.run_dir, 'fakes%06d_1.nii.gz'%(cur_nimg // 1000)), drange=drange_net)
